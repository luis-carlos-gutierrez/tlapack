...
#ifndef TLAPACK_TRTR_QR2_HH
#define TLAPACK_TRTR_QR2_HH
...
void trtr_qr2(matrix_t& A0, matrix_t& A1, vector_t& tau)
{
    ...
    for (idx_t i = 0; i < n - 1; ++i) {
        auto view_A1 = slice(A1, range{0, i}, i);

        larfg(COLUMNWISE_STORAGE, A0(i, i), view_A1, tau[i]);

        auto view_A0 = slice(A0, i, range{i + 1, n});
        auto view_block_A1 = slice(A1, range{0, i}, range{i + 1, n});
